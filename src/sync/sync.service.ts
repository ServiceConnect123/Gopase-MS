...
export class SyncService {
  ...
  async syncRoles(): Promise<SyncResult> {
    ...
    // Protegido: el CRUD de roles ocurre en el backend.
    return this.writeMirrorProtected('roles', rows.map((r) => this.mapRol(r)));
  }
  ...
  async syncEventos(): Promise<SyncResult> {
    ...
    // Protegido: el CRUD de eventos ocurre en el backend.
    return this.writeMirrorProtected('eventos', rows.map((r) => this.mapEvento(r)));
  }
  ...
  async syncAcuerdos(): Promise<SyncResult> {
    ...
    // Protegido: el CRUD de acuerdos ocurre en el backend.
    return this.writeMirrorProtected('acuerdos', rows.map((r) => this.mapAcuerdo(r)));
  }
